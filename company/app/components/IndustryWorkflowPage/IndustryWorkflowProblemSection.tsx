...
export function IndustryWorkflowProblemSection({
  ...
  return (
    <section
      className="grid gap-6 overflow-hidden rounded-lg  bg-white/88 p-[clamp(1.25rem,3vw,2rem)] max-sm:rounded-md max-sm:p-4 min-[56rem]:grid-cols-[0.7fr_1.3fr] min-[56rem]:items-start"
    ...
    >
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,16rem),1fr))] gap-3 [&_svg]:text-(--color-accent)">
        {painPoints.map((point) => (
          <article
            key={point}
            className="rounded-lg border border-slate-900/10 bg-[#fbfcfd] p-6"
          >
